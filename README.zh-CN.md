...
- UHD/DCI 模式会按目标时序更新基础块的 `0xFD` Range Limits 描述符，
  包括垂直/水平频率上下限和最大像素时钟；不再保留原生模板固定的
  59–70 Hz 垂直范围，以免与 24/25/30/50 Hz 的 native VIC 矛盾。
...
# 只检查 10 个 UHD/DCI 模式的范围字段、校验和及 EDID 解码，不写设备：
python3 test_uhd_range.py
```
